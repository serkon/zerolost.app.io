...
export const HostAdd = ({ opened, closed, edit, host }: HostAddProps): React.ReactElement => {
  ...
  const handleCancel = (): void => {
    const value = (host && edit === 'edit' && { ...host, storageType: host?.storageType, password: '' }) || initialValues;
    ...
    <>
      ...
      >
        ...
        <div className="d-flex flex-wrap gap-2 column-gap-4">
          <Select withAsterisk sx={{ flexBasis: '30%', flexGrow: 1 }} type="text" label={translate('TYPE')} placeholder={translate('PLACEHOLDER_TYPE')} name="hostType" data={hostTypeOptions} {...form.getInputProps('storageType')} />
          <TextInput withAsterisk sx={{ flexBasis: '30%', flexGrow: 1 }} type="text" label={translate('VERSION')} placeholder={translate('PLACEHOLDER_VERSION')} name="hostVersion" {...form.getInputProps('storageVersion')} />
