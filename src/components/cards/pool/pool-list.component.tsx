...
export const PoolList = ({ storage }: { storage: string }): React.ReactElement => {
  ...
  const getPoolList = useCallback(() => api.post('/pool/search', { storageId: storage }, { params }), [storage]);
      ...
      .then((items: AxiosResponse<HttpResponse<Pool[]>>) => {
        ...
        if (Pools.length > 0) {
          ...
          if (poolId) {
            ...
            navigate(`/storage/${storage}/${Pools[0].id}`);
          }
        }
      })
      .catch((error) => {
  ...
  const onClickHandler = (pool: Pool): void => {
    ...
    navigate(`/storage/${storage}/${pool.id}`);
  };
  ...
  return (
    <>
      ...
      {pools.length > 0 ? (
        <>
          <ul className="pool-card-list pool-list flex-wrap mx-4 secondary-600">
            {pools.map((pool: any) => (
        ...
        <div className="empty-state mx-4 p-4 dash d-flex flex-grow-1 align-items-center justify-content-center flex-column secondary-400">
