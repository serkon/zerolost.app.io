...
export const Header = (): JSX.Element => {
  ...
  return (
    <header className="d-flex top-header px-4">
      <section className="header-title">
        <Text truncate className="h1 fw-extra-bold secondary-600" sx={(theme):any => ({
      ...
      <section className="actions d-flex align-items-center justify-content-end flex-grow-1">
        ...
        <button className="btn btn-secondary btn-sm btn-ghost me-2"><IconBell size={20} /></button>
          ...
          <Menu.Target>
            <div className="d-flex align-items-center gap-2 user">
              <div className="d-flex flex-column align-items-end gap-1 ms-3">
                <div className="caption-14 fw-semibold secondary-600">{userState.user?.firstName} {userState.user?.lastName}</div>
                <div className="caption-12 secondary-400">{userState.user?.email}</div>
              </div>
              <Avatar color="brand.5" radius="xl">{userState.user?.firstName[0]}{userState.user?.lastName[0]}</Avatar>
            </div>
          </Menu.Target>
          <Menu.Dropdown>
            ...
            <Menu.Item color="red" icon={<IconTrash size={14} />}>{translate('DELETE_MY_ACCOUNT')}</Menu.Item>
            <Menu.Item icon={<IconTrash size={14} />} onClick={():Promise<void> => Authenticator.signOut(() => window.location.reload())}>Logout</Menu.Item>
          </Menu.Dropdown>
        </Menu>
      </section>
    </header>
