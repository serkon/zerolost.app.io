...
export const CheckboxDropdown: React.FC<CheckboxDropdownProps> = ({ options, value, onChange, label, placeholder }) => {
  ...
  return (
    <div ref={ref} className="dropdown d-flex justify-content-end flex-column">
      <label className="dropdown-label d-flex flex-column secondary-600 fw-bold caption-10">
        ...
        <div className="selector position-relative">
          <input className="dropdown-button form-control form-control-lg" onClick={(): void => setIsOpen(!isOpen)} value={`${value.length > 0 ? `${value.length} selected` : ''}`} placeholder={placeholder} readOnly />
          <div className="right-selection">
            <svg width="1.125rem" height="1.125rem" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" data-chevron="true" style={{ color: 'rgb(134, 142, 150)' }}>
              <path
      ...
      </label>
      {
        <div className="dropdown-container">
          {isOpen && (
            <div className={`dropdown-content ${dropdownDirection}`} ref={dropdownContentRef}>
